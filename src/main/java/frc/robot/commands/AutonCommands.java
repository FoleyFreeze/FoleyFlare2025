...
public class AutonCommands {
    ...
    public static Command justDrive(RobotContainer r){
        return DriveCommands.driveForward(r, 0.25).raceWith(new WaitCommand(1)).finallyDo(() -> r.drive.stop());
    }

    public static Command scoreAutoCoral(RobotContainer r){
        SequentialCommandGroup c = new SequentialCommandGroup();
        c.addCommands(DriveCommands.driveForward(r, 0.25).raceWith(new WaitCommand(3)).finallyDo(() -> r.drive.stop()));
